...
def build_generator():
    ...
    normal_2 = tf.keras.layers.BatchNormalization(momentum=0.75)(conv_1)
    ...
    normal_3 = tf.keras.layers.BatchNormalization(momentum=0.75)(conv_2)

    upsample_3 = tf.keras.layers.UpSampling2D(size=(4, 4))(normal_3)  # 64x64
    conv_3 = tf.keras.layers.Conv2DTranspose(128, kernel_size=3, padding='same', activation='relu')(upsample_3)
    normal_4 = tf.keras.layers.BatchNormalization(momentum=0.75)(conv_3)
    ...
    #normal_5 = tf.keras.layers.BatchNormalization(momentum=0.75)(conv_4)

    output = tf.keras.layers.Conv2D(3, kernel_size=3, padding='same', activation='tanh')(normal_4)
