...
def build_generator():
    ...
    label_embedding = tf.keras.layers.Embedding(params.num_classes, params.noise_dim)(label_input)
    label_embedding = tf.keras.layers.Flatten()(label_embedding)

    concatenated_input = tf.keras.layers.Concatenate()([noise_input, label_embedding])

    dense_1 = tf.keras.layers.Dense(256 * 8 * 8, activation='relu')(concatenated_input)
    normal_1 = tf.keras.layers.BatchNormalization(momentum=0.8)(dense_1)
    reshape = tf.keras.layers.Reshape((8, 8, 256))(normal_1)

    upsample_1 = tf.keras.layers.UpSampling2D(size=(2, 2))(reshape)  # 16x16
    conv_1 = tf.keras.layers.Conv2DTranspose(256, kernel_size=3, padding='same', activation='relu')(upsample_1)
    ...
    conv_3 = tf.keras.layers.Conv2DTranspose(64, kernel_size=3, padding='same', activation='relu')(upsample_3)
